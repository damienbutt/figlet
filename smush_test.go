...
func TestCanVerticalSmush(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			"abc", "def",
			makeOpts(layoutFullWidth, false, false, false, false, false),
			"invalid",
		},
		{
			"empty strings → invalid (minLen is 0)",
			"", "",
			makeOpts(layoutFitting, false, false, false, false, false),
			"invalid",
		},
		{
		...
		},
		{
			...
			"a", "b",
			makeOpts(layoutSmushing, false, false, false, false, false),
			"end",
		},
		{
			"controlled smushing, vRule5 (both pipes) → valid (super-smush, loop continues)",
			"|", "|",
			makeOpts(layoutControlledSmushing, false, false, false, false, true),
			"valid",
		},
		{
			"controlled smushing, vRule1 matches (same char) → end",
			"a", "a",
			makeOpts(layoutControlledSmushing, true, false, false, false, false),
			"end",
		},
		{
			"controlled smushing, no rule matches → invalid",
			"a", "b",
			makeOpts(layoutControlledSmushing, false, false, false, false, false),
			"invalid",
		},
		{
